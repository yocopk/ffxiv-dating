  ...
  return (
    <nav className="fixed bg-white shadow-md left-0 right-0 z-10">
      <div className="bg-white flex md:max-w-[1400px] mx-auto justify-between items-center py-3 px-2">
        ...
        <div onClick={() => setOpen(!open)} className="text-red-400 text-3xl">
          <ion-icon name={open ? "close-outline" : "menu-outline"}></ion-icon>
        </div>
      </div>
      <div>
        <ul
          className={`bg-white select-none w-full z-[-1] shadow-md absolute border-t-2 left-0 top-12 p-4 flex flex-col items-center mt-3 transition-all duration-400 ${
        ...
        >
          ...
          {link.map((links) => (
            <li className="p-4" key={links.id}>
              <Link
                className="text-red-400 text-md hover:border-b-2 border-red-400 font-bold"
                to={links.link}
              >
                {links.name}
              </Link>
